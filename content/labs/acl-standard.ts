...
/** Single router, two LANs — standard ACL blocks one subnet from reaching the other. */
export const ACL_STANDARD_LAB: LabSpec = {
  schemaVersion: 1,
  id: "acl-standard",
  ...
  objectives: [
    ...
    "Create `access-list 10 deny 192.168.1.0 0.0.0.255`",
    "Apply `ip access-group 10 out` on Gi0/1 (near destination)",
    "Verify PC1→PC2 ping fails but PC1 can still ping its default gateway",
  ],
  topology: {
    nodes: [
      {
