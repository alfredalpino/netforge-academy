...
test.describe("Simulator inter-vlan-routing lab", () => {
  test("routes between VLANs via router-on-a-stick subinterfaces", async ({ page }) => {
    test.setTimeout(90_000);

    await page.goto("/simulator?lab=inter-vlan-routing");
    await expect(page.getByTestId("simulator-page")).toBeVisible();
    await expect(page.getByText("Inter-VLAN Routing (Router-on-a-Stick)", { exact: true })).toBeVisible();
