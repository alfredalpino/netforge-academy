...
test.describe("Simulator static-route lab", () => {
  test("configures dual routers with static routes and passes grading", async ({ page }) => {
    test.setTimeout(90_000);

    await page.goto("/simulator?lab=static-route");
    await expect(page.getByTestId("simulator-page")).toBeVisible();
    await expect(page.getByText("Static Inter-Network Routing", { exact: true })).toBeVisible();
